    ...
    for k, path in enumerate(rawf):
        ...
        for r in rd["races"]:
            ...
            if len(od) != 120 or not all(o and o > 0 for o in od):
                continue
            try:
                a, b, c = (int(v) - 1 for v in r["hit"].split("-"))
            ...
            rows, _ = S.build_rows(lanes, meta, q, F.FIRST, SECOND, THIRD)
            # ★実際に1着=a・2着=b だった行だけ。これが条件付き確率の定義。
            for j in range(120):
                if F.FIRST[j] != a or SECOND[j] != b:
                    continue
                X.append(rows[j]); Y.append(1 if THIRD[j] == c else 0)
                D.append(int(d))
        if (k + 1) % 200 == 0:
...
THIRD = np.array([int(c[4]) - 1 for c in F.COMBOS])
...
def main():
    ...
    print("\n=== 3着の対数損失（1着2着で条件づけ・4候補）===")
    for nm, mask in (("内側検証", m_iv), ("学習に使っていない期間", ~tr)):
        if mask.sum() < 100:
            continue
        qm = np.exp(X[mask][:, S.FEATS.index("lq3")])
        pm = m.predict(X[mask])
    ...
    m.save_model(f"{args.out}/lgb_3rd.txt")
    with open(f"{args.out}/features_3rd.json", "w", encoding="utf-8") as f:
        json.dump(S.FEATS, f, ensure_ascii=False)
    print(f"\n{args.out}/lgb_3rd.txt に保存しました")
