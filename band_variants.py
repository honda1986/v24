...
for lo, hi in zip(edges[:-1], edges[1:]):
    ...
    print(line + ("  ← 現行の帯 0.12〜0.25 の内訳"
                  if 0.12 <= lo < 0.25 else ""))
...
# 波・風を一切課さない母集団（欠測のレースも含む。§11 の「どちらも省く」と同じ）
...
inq = (q >= 0.12) & (q < 0.25)
# ★欠測は -1 で入っている。「波<3」だけ書くと欠測(-1)が通ってしまうので、
#   使う変数についてだけ「値がある」ことを要求する。両方の存在を求めると、
#   「波だけ残す」案に風の欠測まで効いてしまい、比べている母集団がずれる。
POP = {
    "現行(波0-2+風<4)": OK & inq,
    "波だけ残す(波0-2)": (~tansui) & (C["wave"] >= 0) & (C["wave"] < 3) & inq,
    "風だけ残す(風<4)": (~tansui) & (C["wind"] >= 0) & (C["wind"] < 4) & inq,
    "どちらも省く": (~tansui) & inq,
}
...
print("\n\n############ 13. フォールバックのしきい値（band_howto.md §12）############")
print("   本番は g+h。片方が欠けたときに何で買うかを、同じ2期間で決める。")
if "p_h" not in C:
    print("   ★このダンプに p_h 列がありません。ana_dump.py を作り直してください")
else:
    VAR = {"g のみ": C["p_g"] / q, "h のみ": C["p_h"] / q,
           "補正なし": C["p_base"] / q}
    tgt = int((OK & (q >= 0.12) & (q < 0.25)
               & (PQ["g+h"] > 1.15) & (X_ | B_)).sum())
    print(f"   基準: g+h / p/q>1.15 は通し {tgt:,}点")
    for key, v in VAR.items():
        print(f"\n   --- {key} ---")
        print(f"     {'p/q>':>6} |{'探索':>18}|{'独立':>18}|"
              f"{'通し 点数 回収率 95%区間 P':>38}")
        for t in (1.05, 1.078, 1.09, 1.10, 1.11, 1.12, 1.13, 1.14, 1.15, 1.18):
            line = f"     {t:6.3f} |"
            rs = []
            for mk in (X_, B_):
                m = mk & OK & (q >= 0.12) & (q < 0.25) & (v > t)
                nr, nb, nh, r = roi(m)
                rs.append(r)
                line += f" {nb:5,} {r:6.1f}% |"
            m = (X_ | B_) & OK & (q >= 0.12) & (q < 0.25) & (v > t)
            nr, nb, nh, r = roi(m)
            (lo, hi), pw = boot(m)
            print(line + f" {nb:5,}点 {r:6.1f}% [{lo:3.0f},{hi:3.0f}] P={pw:.2f}"
                  + ("  ◎" if rs[0] > 100 and rs[1] > 100 else ""))
        idx = np.where(OK & (q >= 0.12) & (q < 0.25) & (X_ | B_))[0]
        srt = idx[np.argsort(-v[idx])[:tgt]]
        print(f"     → g+h/1.15 と同じ {tgt:,}点にするしきい値 "
              f"p/q>{v[srt].min():.3f}")
